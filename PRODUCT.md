...
- 长文按公众号平台上界量级（约 2 万字、**约 600 块**，该量级经真实样例修正）设计，连续渲染不做虚拟列表，不设数量硬上限；超过 1000 块只提示不拒绝。完整性能契约见[长文性能决策](https://github.com/dulltackle/WeTrim/issues/11#issuecomment-5595575026)。
...
规范来源为根目录 `SPEC.md`，它由[浏览器扩展决策地图](https://github.com/dulltackle/WeTrim/issues/1)的全部决策票整合而成；旧的 `mvp.md` 已删除。原型使用明确标记的合成文章与模拟状态。性能目标出自一次可丢弃探针的**合成文章**实测，`chrome.storage.local` 写入与搜索筛选均未实测；真实长文样例已由[收集任务](https://github.com/dulltackle/WeTrim/issues/12)补齐，连同[结构覆盖任务](https://github.com/dulltackle/WeTrim/issues/13)的四篇一起作为实现期验收基准；完整的「不得当作已验证」清单见 `SPEC.md` 第 12 节。
